# Hi, I'm Noor Ul Ain Jellani 👋

### 🎓 CS Undergrad @ IST Islamabad
### 💡 Passionate about merging logic with creativity, dedicated to building intuitive and efficient solutions.

---

## 🚀 About Me

I'm a driven Computer Science undergraduate at IST Islamabad, with a keen interest in **Computer vision and **problem-solving using C++**. I'm passionate about transforming ideas into functional and user-friendly applications. My journey involves continuously learning new technologies and strengthening my foundational skills to create impactful digital experiences. I thrive on challenges and am always eager to explore innovative solutions.

---

## 🛠️ Tech Stack & Skills

Here's a snapshot of the technologies and tools I'm proficient with or actively learning:

* **Languages**: C++, JavaScript
* **Web Development**: HTML, CSS.
* **Databases**: SQL (experience with basic database design & queries)
* **Version Control**: Git, GitHub
* **Tools**: VS Code
* **Operating Systems**:Mac

---

## 🔥 Current Focus & Learning

* **Strengthening Fundamentals**: Deepening my understanding of Data Structures and Algorithms in C++.
* **Web Development**: Actively building **JavaScript mini-games** and developing a personal **portfolio website** to refine my front-end skills.
* **Consistency**: Maintaining a strong **GitHub daily push streak** to track and enhance my coding consistency.
* **Problem-Solving**: Constantly improving my problem-solving abilities through competitive programming and LeetCode challenges.

---

## 📊 My GitHub Activity

Take a look at my recent contributions and overall stats!

[![GitHub Streak](https://streak-stats.demolab.com/?user=nurulain0&theme=tokyonight&hide_border=true)](https://git.io/streak-stats)
[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=nurulain0&show_icons=true&theme=tokyonight&hide_border=true&count_private=true)](https://github.com/anuraghazra/github-readme-stats)
[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=nurulain0&layout=compact&theme=tokyonight&hide_border=true)](https://github.com/anuraghazra/github-readme-stats)

---
✨ Code in Motion

![Coding GIF](https://media.giphy.com/media/LMt63f25c7z8k/giphy.gif)


## 🤝 Let's Connect!

I'm always open to discussing new ideas, collaborating on interesting projects, or just connecting with fellow developers. Feel free to reach out!

* **GitHub**: [github.com/nurulain0](https://github.com/nurulain0)
* **LinkedIn**: www.linkedin.com/in/noor-ul-ain-jellani-gujar-54b54513b
* **Email**: noorjellani98@gmail.com 

---

## 📈 What I'm Looking For
* **Collaborations**: Interested in contributing to open-source projects or collaborating on innovative web development ideas.
